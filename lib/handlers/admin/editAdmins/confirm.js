...
const removeHandler = async username => {
  ...
  return { userId: id, chatId, message: successRemoveAdmin, userMessage: youRemovedFromAdmins, isAdmin: false }
}
...
module.exports = async ctx => {
  ...
  const { chatId, message, userMessage, isAdmin } = await (async () => {
  ...
  await ctx.telegram.sendMessage(chatId, userMessage(ctx, { username: from.username }), coreKeyboard(ctx, isAdmin))
