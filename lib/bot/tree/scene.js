...
const { PARENT_SCENE } = require('./constants')
...
class Scene extends Composer {
  ...
  static jump(scene) {
    return ctx => ctx.scene.enter(scene)
  }

  static root() {
    return ctx => ctx.scene.root()
  }

  static parent() {
    return ctx => ctx.scene.parent()
  }

  static back() {
    return ctx.ctx.scene.back()
  }

  static child(scene) {
    return ctx => ctx.scene.child(scene)
  }
}
