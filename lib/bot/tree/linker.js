...
const { LINKER_ROOT_SCENE, IS_ROOT_SCENE } = require('./constants')

class SceneLinker {
  constructor() {
    this.scenes = new Map()
    this.paths = new Map()
    this[LINKER_ROOT_SCENE] = null
  }

  set root(scene) {
    if (!(scene instanceof Scene)) throw new Error('The root must be an instance of the Scene.')
    if (scene.parent) throw new Error(`The root must not have a parent. [scene: ${scene.id}, parent: ${scene.parent.id}]`)
    if (this[LINKER_ROOT_SCENE])
      throw new Error(`You cannot add root twice. [current: ${this[LINKER_ROOT_SCENE].id}, next: ${scene.id}]`)

    this[LINKER_ROOT_SCENE] = scene
  }

  get root() {
    if (!this[LINKER_ROOT_SCENE]) throw new Error('Root scene not found!')
    return this[LINKER_ROOT_SCENE]
  }

  add(scene) {
    if (!(scene instanceof Scene)) throw new Error('Incorrect scene')
    this.scenes.set(scene.id, scene)
    if (scene[IS_ROOT_SCENE]) this.root = scene
  }
  ...
  isChild(_parent, _child) {
    const parent = this.get(_parent)
    const child = this.get(_child)
    return !!(parent && child && parent.childs.has(child.id))
  }
  ...
  static getId(scene, stringReturn) {
    ...
    if (scene instanceof Scene) return this.getId(scene.id, stringReturn)
