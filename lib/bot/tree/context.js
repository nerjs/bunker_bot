...
const SceneContextRefer = require('./refer')
...
class SceneContext {
  ...
  set refer(value) {
    if (!value || typeof value !== 'object') {
      ...
      this.__refer = new SceneContextRefer(this, value)
    }
  }

  get refer() {
    return this.__refer || new SceneContextRefer(this)
  }
  ...
  async enter(sceneId, currentMessage) {
    ...
    console.log(this.current.id, scene.id, scene === this.current)
    if (scene === this.current && !(await this.current.canReenter(this.ctx))) return

    this.refer = {
      from: (scene === this.current && (this.current.parent || this.refer.from)) || this.current,
      to: scene,
      move: true,
    }

    if (down.length || this.linker.isChild(this.refer.to, this.refer.from)) {
    ...
    if (this.refer.from !== this.current) {
      await this.current.sendHeaders(this.ctx, currentMessage, !this.refer.stopped)
    } else if (!this.refer.stopped) {
