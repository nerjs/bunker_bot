...
class SceneContext {
  ...
  hasBack() {
    return !this.isMooving() && !!this.refer.from
  }
  ...
  stop() {
    if (this.isMooving()) this.refer.stopped = true
  }

  jump(scene) {
    return this.enter(scene)
  }

  root() {
    return this.enter(this.linker.root)
  }

  parent() {
    if (this.current.parent) return this.enter(this.current.parent)
  }

  back() {
    if (this.hasBack()) return this.enter(this.refer.from)
  }
