...
const createMLC = (langSchema = {}) => {
  ...
  mlcGetter.is = txt => mlcGetter.regexp.test(txt)
  ...
  mlcGetter.middleware = async (ctx, next) => {
    ...
    if ((ctx.match.groups && ctx.match.groups[userLang]) || (!mlcGetter.languages[userLang] && ctx.match[DEFAULT_LANG]))
      return next(ctx)
  }

  mlcGetter[IS_MLC] = true
  mlcGetter[IS_MODIFIED] = true
