...
const createML = langSchema => {
  const mlGetter = (lang, ctx, params = {}) => {
    if (isContext(lang)) return mlGetter(lang.from.language_code, lang, ctx)
    if (typeof lang === 'object') return mlGetter(DEFAULT_LANG, lang, ctx)

    const result = langSchema[lang] || langSchema[DEFAULT_LANG]
    return typeof result === 'function' ? result(ctx, params) : result
  }
