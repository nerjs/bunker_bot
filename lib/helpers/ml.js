...
const createML = langSchema => {
  const mlGetter = lang => {
    if (isContext(lang)) return mlGetter(lang.from.language_code)
    return langSchema[lang] || langSchema[DEFAULT_LANG]
  }
...
const ml = _langSchema => {
  ...
  if (typeof _langSchema === 'string') return ml({ [DEFAULT_LANG]: _langSchema })
